...
def calculate_body_fat_percentage(weight, height, age, gender, waist, neck=0, hip=0):
    ...
    if gender == '男':
        ...
        if neck_inches <= 0:
            return None
        body_fat_percentage = 86.010 * (waist_inches - neck_inches) / height_inches - 70.041 * (
                    height_inches / 100) + 36.76
    else:
        # 女性体脂率计算公式
        hip_inches = hip * 0.393701 if hip > 0 else 0
        if neck_inches <= 0 or hip_inches <= 0:
            return None
        body_fat_percentage = 163.205 * (waist_inches + hip_inches - neck_inches) / height_inches - 97.684 * (
                    height_inches / 100) - 78.387
